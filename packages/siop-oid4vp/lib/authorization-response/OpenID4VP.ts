...
export const verifyPresentations = async (
  ...
  const nonMdocPresentations = presentationsArray.filter((p) => p.format !== 'mso_mdoc')
  const nonces = new Set(nonMdocPresentations.map(extractNonceFromWrappedVerifiablePresentation))
